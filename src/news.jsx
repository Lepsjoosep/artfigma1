import { Link } from 'react-router-dom';

function News() {
    return (
         <div className='flex flex-row justify-between'>
      <div>
        <div className='flex flex-row p-10 pl-10'>
          <img src="./src/Pictures/Hopedieslast.jpg" alt="" className='h-50 w-50 object-cover' />
          <div className='flex flex-col pl-10 lg:pl-5 '>
            <h1 className='text-3xl lg:text-2xl font-bold pb-5'>Hope dies last</h1>
            <h1 className='w-190'>Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Egestas dui id ornare arcu odio ut sem. Cras ornare arcu dui vivamus arcu felis bibendum ut. Porttitor leo a diam.</h1>
            <div className='flex flex-row gap-5 align-center pt-10'>
              <div className='flex flex-row gap-3'>
                <h1 className='font-bold'>Text</h1>
                <h1>Jakob Gronberg</h1>
              </div>
              <div className='flex flex-row gap-3'>
                <h1 className='font-bold'>Date</h1>
                <h1>16.March 2022</h1>
              </div>
              <div className='flex flex-row gap-3'>
                <h1 className='font-bold'>Duration</h1>
                <h1>1 Min</h1>
                <div>
                  <Link to="/magazine"><button className='w-13 h-8 border-1 rounded-2xl ml-50 lg:ml-40'>ART</button></Link>
                 </div>
              </div>
            </div>
          </div>
        </div>
        <hr className='bg-black w-250 border-1 ml-10' />
        <div className='flex flex-row p-10 pl-10'>
          <img src="./src/Pictures/thebest.jpg" alt="" className='h-50 w-50 object-cover' />
          <div className='flex flex-col pl-10 '>
            <h1 className='text-3xl font-bold pb-5'>The best art museums</h1>
            <h1 className='w-190'>Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Egestas dui id ornare arcu odio ut sem. Cras ornare arcu dui vivamus arcu felis bibendum ut. Porttitor leo a diam.</h1>
            <div className='flex flex-row gap-5 align-center pt-10'>
              <div className='flex flex-row gap-3'>
                <h1 className='font-bold'>Text</h1>
                <h1>Jakob Gronberg</h1>
              </div>
              <div className='flex flex-row gap-3'>
                <h1 className='font-bold'>Date</h1>
                <h1>16.March 2022</h1>
              </div>
              <div className='flex flex-row gap-3'>
                <h1 className='font-bold'>Duration</h1>
                <h1>1 Min</h1>
                <div>
                  <Link to="/magazine"><button className='w-30 h-8 border-1 rounded-2xl ml-35 lg:ml-25'>SCULPTURES</button></Link>
                </div>
              </div>
            </div>
          </div>
        </div>
        <hr className='bg-black w-250 border-1 m-10' />
        <div className='flex flex-row p-10 pl-10'>
          <img src="./src/Pictures/thedevil.jpg" alt="" className='h-50 w-50 object-cover' />
          <div className='flex flex-col pl-10 '>
            <h1 className='text-3xl font-bold pb-5'>The devil is the details</h1>
            <h1 className='w-190'>Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Egestas dui id ornare arcu odio ut sem. Cras ornare arcu dui vivamus arcu felis bibendum ut. Porttitor leo a diam.</h1>
            <div className='flex flex-row gap-5 align-center pt-10'>
              <div className='flex flex-row gap-3'>
                <h1 className='font-bold'>Text</h1>
                <h1>Jakob Gronberg</h1>
              </div>
              <div className='flex flex-row gap-3'>
                <h1 className='font-bold'>Date</h1>
                <h1>16.March 2022</h1>
              </div>
              <div className='flex flex-row gap-3'>
                <h1 className='font-bold'>Duration</h1>
                <h1>1 Min</h1>
                <div>
                  <Link to="/magazine"><button className='w-13 h-8 border-1 rounded-2xl ml-50 lg:ml-40'>ART</button></Link>
                </div>
              </div>
            </div>
          </div>
        </div>
        <hr className='bg-black w-250 border-1 ml-10' />
        <div className='flex flex-row p-10 pl-10'>
          <img src="./src/Pictures/hope.jpg" alt="" className='h-50 w-50 object-cover' />
          <div className='flex flex-col pl-10 '>
            <h1 className='text-3xl font-bold pb-5'>An indestructible hope</h1>
            <h1 className='w-190'>Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Egestas dui id ornare arcu odio ut sem. Cras ornare arcu dui vivamus arcu felis bibendum ut. Porttitor leo a diam.</h1>
            <div className='flex flex-row gap-5 align-center pt-10'>
              <div className='flex flex-row gap-3'>
                <h1 className='font-bold'>Text</h1>
                <h1>Jakob Gronberg</h1>
              </div>
              <div className='flex flex-row gap-3'>
                <h1 className='font-bold'>Date</h1>
                <h1>16.March 2022</h1>
              </div>
              <div className='flex flex-row gap-3'>
                <h1 className='font-bold'>Duration</h1>
                <h1>1 Min</h1>
                <div>
                  <Link to="/magazine"><button className='w-13 h-8 border-1 rounded-2xl ml-50 lg:ml-40'>ART</button></Link>
                </div>
              </div>
            </div>
          </div>
        </div>
        <hr className='bg-black w-250 border-1 ml-10' />
        <div className='flex flex-row p-10 pl-10'>
          <img src="./src/Pictures/streetart.jpg" alt="" className='h-50 w-50 object-cover' />
          <div className='flex flex-col pl-10 '>
            <h1 className='text-3xl font-bold pb-5'>Street art festival</h1>
            <h1 className='w-190'>Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Egestas dui id ornare arcu odio ut sem. Cras ornare arcu dui vivamus arcu felis bibendum ut. Porttitor leo a diam.</h1>
            <div className='flex flex-row gap-5 align-center pt-10'>
              <div className='flex flex-row gap-3'>
                <h1 className='font-bold'>Text</h1>
                <h1>Jakob Gronberg</h1>
              </div>
              <div className='flex flex-row gap-3'>
                <h1 className='font-bold'>Date</h1>
                <h1>16.March 2022</h1>
              </div>
              <div className='flex flex-row gap-3'>
                <h1 className='font-bold'>Duration</h1>
                <h1>1 Min</h1>
                <div>
                  <Link to="/magazine"><button className='w-30 h-8 border-1 rounded-2xl ml-35 lg:ml-25'>STREET ART</button></Link>
                </div>
              </div>
            </div>
          </div>
        </div>
        <hr className='bg-black w-250 border-1 ml-10' />
        <div className='flex flex-row p-10  pl-10'>
          <img src="./src/Pictures/streetartists.jpg" alt="" className='h-50 w-50 object-cover' />
          <div className='flex flex-col pl-10 '>
            <h1 className='text-3xl font-bold pb-5'>Through the eyes of street artists</h1>
            <h1 className='w-190'>Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Egestas dui id ornare arcu odio ut sem. Cras ornare arcu dui vivamus arcu felis bibendum ut. Porttitor leo a diam.</h1>
            <div className='flex flex-row gap-5 align-center pt-10'>
              <div className='flex flex-row gap-3'>
                <h1 className='font-bold'>Text</h1>
                <h1>Jakob Gronberg</h1>
              </div>
              <div className='flex flex-row gap-3'>
                <h1 className='font-bold'>Date</h1>
                <h1>16.March 2022</h1>
              </div>
              <div className='flex flex-row gap-3'>
                <h1 className='font-bold'>Duration</h1>
                <h1>1 Min</h1>
                <div>
                  <Link to="/magazine"><button className='w-30 h-8 border-1 rounded-2xl ml-35 lg:ml-25'>STREET ART</button></Link>
                </div>
              </div>
            </div>
          </div>
        </div>
        <Link to="/magazine"><div className='p-10 pl-10 flex flex-row gap-2 align-center'>
          <h1 className='font-bold'>ALL ARTICLES</h1>
          <img src="./src/Pictures/arrow.png" alt=""  className='h-5'/>
        </div></Link>
      </div>
      <div className='p-10 pr-20'>
        <div className='pb-10'>
          <h1 className='font-bold'>PRINTMAGAZINE</h1>
          <h1 className='font-bold text-5xl overflow-hidden'>03/2022</h1>
        </div>
        <img src="./src/Pictures/magazinecover.png" alt="" className='h-110' />
        <button className='bg-black text-white mt-3 h-12 w-25'>BUTTON</button>
        <div className='pt-15'>
          <h1 className='font-bold text-lg'>MOST POPULAR</h1>
          <div className='flex flex-row gap-8 pt-5'>
            <h1 className='font-bold text-2xl'>01</h1>
            <div className='flex flex-col gap-3'>
              <h1 className='font-bold text-2xl overflow-hidden'>Street art festival</h1>
              <div className='flex flex-row gap-3'>
                <h1 className='font-bold'>Text</h1>
                <h1>Cristofer Vaccaro</h1>
              </div>
            </div>
          </div>
          <hr className='bg-black mt-5 mr-10' />
          <div className='flex flex-row gap-8 pt-5'>
            <h1 className='font-bold text-2xl'>02</h1>
            <div className='flex flex-col gap-3'>
              <h1 className='font-bold text-2xl overflow-hidden'>Hope dies last</h1>
              <div className='flex flex-row gap-3'>
                <h1 className='font-bold'>Text</h1>
                <h1>Anne Henry</h1>
              </div>
            </div>
          </div>
          <hr className='bg-black mt-5 mr-10' />
          <div className='flex flex-row gap-8 pt-5'>
            <h1 className='font-bold text-2xl'>03</h1>
            <div className='flex flex-col gap-3'>
              <h1 className='font-bold text-2xl overflow-hidden w-70'>Artists who want to rise above</h1>
              <div className='flex flex-row gap-3'>
                <h1 className='font-bold'>Text</h1>
                <h1>Anne Nielsen</h1>
              </div>
            </div>
          </div>
        </div>
        <div className='bg-[#F8F8F8] mt-20 w-100 p-8 gap-3 flex flex-col'>
          <h1 className='font-bold text-lg'>NEWSLETTER</h1>
          <h1 className='font-bold text-4xl w-70 h-22'>Design News to your inbox</h1>
          <input type="text" placeholder='Email' className='bg-white p-2 mt-5 max-w'/>
          <div className='flex justify-end'>
            <button className='bg-black text-white w-30 h-15'>SIGN UP</button>
          </div>
        </div>
      </div>
    </div>
    );
}
export default News;